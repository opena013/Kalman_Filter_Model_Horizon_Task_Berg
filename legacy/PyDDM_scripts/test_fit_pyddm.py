...
social_media_df = pd.DataFrame({
    'game_number': pd.Series(range(1, 41)).repeat(9).values,
    'gameLength': df['gameLength'].repeat(9).values,
    'trial': np.tile(trial_nums, len(df)),
    ...
    'left_bandit_outcome': df[[f'mu1_reward{i}' for i in trial_nums]].values.flatten(),
    'right_bandit_outcome': df[[f'mu2_reward{i}' for i in trial_nums]].values.flatten(),
    'drift_value': np.tile(trial_nums, len(df)) / 7,
    'starting_position_value': np.tile(trial_nums, len(df)) / 10
})
...
model_to_fit = pyddm.gddm(drift=lambda drift_dcsn_noise_mod,drift_value,sigma_d,sigma_r,baseline_noise,side_bias,directed_exp,baseline_info_bonus,random_exp,rel_uncert_mod : drift_value*.1,
                          starting_position=lambda starting_position_value: starting_position_value, 
...
model_to_fit.fit(sample=social_media_sample,verbose=True)
