function [fit_info, model_output] = fit_extended_model_no_latent_state_learning(formatted_file, result_dir, MDP)
    if ispc
        root = 'L:/';
    else
        root = '/media/labs/';
    ...
    in = struct(...
        ...
        'bandit2_schedule', sub.bandit2_schedule, 'MDP', MDP, 'result_dir', result_dir,'get_summary_stats',0,'sim',0);
    ...
    % Get only free choices
    free_choices = sub.a(:,5:end);
    free_choices_row =  [reshape(free_choices.', 1, [])] - 1;
    y = free_choices_row(~isnan(free_choices_row)); % remove NaN values
    ...
    % f_fname = @f_KF_SIGMA4;
    g_fname = @g_KF_SIGMA4_logistic_no_latent_state_learning;

    

    dim = struct( ...
        'n', 0, ... number of hidden states 
    ...
    % assemble actions variable y
    % 0 indicates the left bandit was chosen, 1 indicates the right
    free_choices = simmed_model_output.actions(:,5:end);
    free_choices_row =  [reshape(free_choices.', 1, [])] - 1;
    y = free_choices_row(~isnan(free_choices_row)); % remove NaN values
