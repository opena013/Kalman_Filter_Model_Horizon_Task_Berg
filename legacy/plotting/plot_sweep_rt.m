function plot_sweep_rt(simmed_model_output,study_info)
    ...
    % === SETUP ===
    fig = figure('Name','Parameter Sweep RT','Position',[100, 100, 1200, 700]);
    ...
    title(ax, ['Mean RT across values of ', sweep_param]);
    grid(ax, 'on');
    ...
    for h = horizons
        for c = choice_nums
            line_idx = line_idx + 1;
            col_mean = sprintf('mean_rt_hor%d', h);
            col_std  = sprintf('std_rt_hor%d', h);
