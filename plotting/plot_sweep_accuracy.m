function plot_sweep_accuracy(simmed_model_output,study_info)
    ...
    ylabel(ax, 'Accuracy (P[Choose Correct])');
    title(ax, ['Accuracy across values of ', sweep_param]);
    grid(ax, 'on');

    % Store valid lines and labels
    line_pairs = {};  % {main_line, dummy_line_for_structure}
    labels = {};
    checkbox_idx = 0;
    line_idx = 0;
    ...
    for h = horizons
        for c = choice_nums
            line_idx = line_idx + 1;
            col_mean = sprintf('mean_prob_choose_cor_hor%d', h);
            col_std  = sprintf('std_prob_choose_cor_hor%d', h);
            ...
            line_pairs{checkbox_idx} = {h_line, gobjects(1)}; % dummy second handle
